...
def upload(user_id):
    ...
    # TODO: implement proper  error checking to see if the firebase stored it properly
    result = storage.child(name).put(image)
    ...
    image = Images.query.order_by(Images.timestamp).filter(Images.image == name).one_or_none()
...
def get_image(user_id,image_id):
    ...
    user = firebase.auth().sign_in_with_email_and_password('admin@gmail.com','adminuser')
